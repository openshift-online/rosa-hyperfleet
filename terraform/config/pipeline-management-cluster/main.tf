  ...
  trigger {
    ...
    git_configuration {
      ...
      push {
        ...
        file_paths {
          includes = ["deploy/${var.target_environment}/${var.target_region}/pipeline-management-cluster-${local.name_prefix}-inputs/terraform.json", "terraform/config/pipeline-management-cluster/**", "terraform/modules/hypershift-oidc/**"]
        }
      }
    }
  }
