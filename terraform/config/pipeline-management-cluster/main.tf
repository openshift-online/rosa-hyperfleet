...
locals {
  ...
  kube_applier_dynamodb_project_name = "${local.name_prefix}-kube-applier-dynamodb"
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.management_apply.name}",
          "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.management_apply.name}:*",
          "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.management_bootstrap.name}",
          "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.management_bootstrap.name}:*",
          "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.register.name}",
          "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.register.name}:*",
          "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.kube_applier_dynamodb.name}",
          "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.kube_applier_dynamodb.name}:*"
        ]
      },
      {
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          ...
          aws_codebuild_project.kube_applier_dynamodb.arn,
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:codebuild:*:*:project/${aws_codebuild_project.kube_applier_dynamodb.name}",
...
resource "aws_codebuild_project" "kube_applier_dynamodb" {
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = "terraform/config/pipeline-management-cluster/buildspec-provision-kube-applier-dynamodb.yml"
  }
}
...
resource "aws_codepipeline" "regional_pipeline" {
  ...
  stage {
    name = "Provision-KubeApplier-DynamoDB"

    action {
      name            = "KubeApplierDynamoDB"
      category        = "Build"
      owner           = "AWS"
      provider        = "CodeBuild"
      ...
      configuration = {
        ProjectName = aws_codebuild_project.kube_applier_dynamodb.name
        EnvironmentVariables = jsonencode([
          {
            name  = "IS_DESTROY"
            value = "#{variables.IS_DESTROY}"
